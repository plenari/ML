...
def log_sum_exp(vec):
    '''
    计算矢量的log(sum(exp(vec)))
    vec[0]对应什么
    vec的最大值对应什么,是不是标签的类别？
    '''
...
class BiLSTM_CRF(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim):
        ...
        self.tag_to_ix = tag_to_ix#标签 ，都是 BIO，开始，终点
    ...
    def _score_sentence(self, feats, tags):
        ...
        #第一个和start转移概率，转移矩阵是从行转移到列。
        for i, feat in enumerate(feats):
            #所有概率之和，第i格单词的特征对应第i+1个转移矩阵，和改单词的特征之和。
            score = score + \
    ...
    def _viterbi_decode(self, feats):
        '''
        微比特算法，求解最有路程。这个问题就是HMM.
        1. 观测状态，即句子内的单词
        2. 隐藏状态，即单词的标记，比如BIo，start,end
        3. feats 所有单词可能的状态，每行代表一个单词
        还需要标签，转移矩阵，势能矩阵等等
        '''
        backpointers = []
...
with torch.no_grad():
    ...
    print('percheck_sent',model(precheck_sent))
